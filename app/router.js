...
module.exports = app => {
  ...
  router.post('/user/list',new Auth(1).check,controller.user.list);// 用户列表获取-done   new Auth(1).check
  ...
  router.get('/application/detail',new Auth(1).check,controller.application.detail);//应用详情的查看
  ...
  router.get('/application/indicatorNum',new Auth(1).check,controller.application.indicatorNum);//应用下是否有指标
  ...
  //标签
  router.post('/label/list',new Auth(1).check,controller.theLabel.list);// 标签的列表的获取
